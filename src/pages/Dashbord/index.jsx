...
function reducer(state, action) {
    switch (action.type) {
        ...
        case 'EDIT_PARTNER':
            return { ...state, partners: state.partners.map(partner => partner.id === action.payload.id ? action.payload : partner) };
        case 'DELETE_PARTNER':
...
const Dashbord = () => {
    ...
    const handleAddPartner = (newPartner) => {
        dispatch({ type: 'ADD_PARTNER', payload: { id: partners.length + 1, ...newPartner } });
    };

    const handleEditPartner = (updatedPartner) => {
        dispatch({ type: 'EDIT_PARTNER', payload: updatedPartner });
    };
